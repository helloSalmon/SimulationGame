...
{
    public List<Color> colorList = new List<Color> { Color.red, Color.blue, Color.green };
    List<string> companyCodeList = new List<string> { "AAA", "BBB", "CCC", "DDD" };

    List<Sprite> companySprite = new List<Sprite>();
    ...
    void Start()
    {
        for (int i = 1; i <= 4; ++i)
        {
            Sprite sprite = Managers.Resource.Load<Sprite>($"Patterns/{i}");
            companySprite.Add(sprite);
        }
